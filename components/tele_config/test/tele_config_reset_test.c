...
int main(void)
{
    ...
    assert(tele_config_reset_value("wifi.sta_max_retry", &result) == ESP_OK);
    assert(callback_calls == 1);
    assert(callback_value.u32 == 3);
    assert(result.applied == true);
    assert(result.stored == false);
    assert(result.requires_reboot == false);

    result = (tele_config_update_result_t){0};
    assert(tele_config_reset_value("wifi.apsta_policy", &result) == ESP_OK);
    assert(callback_calls == 1);
    assert(result.applied == false);
    assert(result.stored == false);
    assert(result.requires_reboot == true);

    assert(tele_config_reset_value("missing.field", NULL) == ESP_ERR_INVALID_ARG);
    return 0;
}
